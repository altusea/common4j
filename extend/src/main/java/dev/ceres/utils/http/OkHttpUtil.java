package dev.ceres.utils.http;

import cn.hutool.core.net.url.UrlBuilder;
import dev.ceres.utils.json.JacksonUtil;
...
public class OkHttpUtil {
    ...
    public static <R> R postForEntity(String url, Object obj, Class<R> responseType, Proxy proxy) {
        RequestBody requestBody = RequestBody.create(JacksonUtil.toJson(obj), JSON);
        Request request = new Request.Builder()
                .url(url)
                .post(requestBody)
                .build();
    ...
    private static <R> R exec(Request request, Class<R> responseType, OkHttpClient httpClient) {
        ...
        try (Response response = failsafeCall.execute()) {
            ...
            return JacksonUtil.fromJson(responseBody.string(), responseType);
        } catch (IOException e) {
